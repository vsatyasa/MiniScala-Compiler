...
object CMScalaType {
  ...
  case object ExistsType extends Type
  ...
  case class FunType(reftp: List[Type], args: List[(String, Type)], rtp: Type) extends Type {
    val srtp = rtp match {
      case FunType(_, _, _) => s"($rtp)"
      case _ => s"$rtp"
    }
  ...
  def errorType(tp: Type): Type = tp match {
    case UnknownType => ErrorType
    case NothingType => ErrorType
    case FunType(ref, args, rtp) => FunType(ref, args map { x => (x._1, errorType(x._2)) }, errorType(rtp))
    case BlockType(tpl, tags) => BlockType(tpl map(errorType), tags)
    case _ => tp
  }
  ...
  val isKnownType = Set[Type](IntType, BooleanType, CharType, UnitType, AnyType, NothingType, ErrorType)
