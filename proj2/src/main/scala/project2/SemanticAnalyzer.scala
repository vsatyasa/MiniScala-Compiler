...
class SemanticAnalyzer(parser: Parser) extends Reporter {
  ...
  def analyzeDec(x: String, exp: Exp)(env: VarEnv): Unit = {
      if (env.isDefined(x)) warn(s"re declaring the variable ${x}", exp.pos)
        // if (env.isVar(x)) warn(s"re declaring the variable ${x}", exp.pos)
        // else error(s"re-assignment to val ${x}", exp.pos)
  }
  ...
  def analyze(exp: Exp)(env: VarEnv): Unit = exp match {
    ...
    case Prim(op, lop, rop) =>
      ...
      analyze(rop)(env)
    case Let(x, a, b) =>
      analyzeDec(x, exp)(env)
      analyze(a)(env)
      analyze(b)(env.withVal(x))
    case Ref(x) =>
      if (!env.isDefined(x)) error(s"undefined identifier ${x}", exp.pos)
    case Cond(op, l, r) =>
      if (!isBOperator(op))
    ...
    case VarAssign(x, rhs) =>
      if (env.isDefined(x)) {
        if (!env.isVar(x)) error(s"re-assignment to val ${x}", exp.pos) 
      }
      else error(s"undefined identifier ${x}", exp.pos)
      analyze(rhs)(env)
