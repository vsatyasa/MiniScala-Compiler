...
def makeRNG(seed: Int):Array[Int] = {
  val rng = new Array[Int](1);
  rng(0) = seed & 0xFFFF;
  rng
};

def rngGetState(rng: Array[Int]): Int = rng(0);

def rngSetState(rng: Array[Int], state: Int): Unit = rng(0) = state & 0xFFFF;

def rngRotateRight8(x: Int, y: Int):Int =
  ((x >> y) | (x << (8 - y))) & 0xFF;

def rngStep(rng: Array[Int]): Unit =
  rngSetState(rng, rngGetState(rng) * 12829 + 47989);

def rngOutput(rng: Array[Int]):Int = {
...
def rngNextInt8(rng: Array[Int]):Int = {
  val i = rngOutput(rng);
  rngStep(rng);
  i
};
...
def rngNextInt(rng: Array[Int]):Int = {
