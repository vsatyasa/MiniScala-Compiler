...
class SemanticAnalyzer(parser: Parser) extends MiniScalaReporter with BugMiniScalaReporter {
  ...
  def funType(ftps: List[String], args: List[N.Arg], rtp: Type): FunType = {
    FunType(ftps map(RefType(_)), args.map { arg => (arg.name, arg.tp) }, rtp)
  }

  def listArgType(size: Int, tp: Type) = List.fill(size)(("$Args", tp))
  ...
  def typeNuOperator(op: String)(pos: Pos) = op match {
    case "byte-read" => FunType(Nil, Nil, IntType)
    case _ =>
  ...
  def typeUnOperator(op: String)(pos: Pos) = op match {
    case _ if isIntUnOperator(op) => FunType(Nil, List(("", IntType)), IntType)
    case "byte-write" => FunType(Nil, List(("", IntType)), UnitType)
    case _ =>
  ...
  def anySelect(t: S.Tree, m: String, pt: Type, pos: Pos): S.Tree = {
    m match {
      ...
      case _ =>
        ...
        S.Prim(ErrorPrim, Nil).withType(errorType(pt))
    }
  }
  ...
  def typeBinOperator(op: String)(pos: Pos) = op match {
    case _ if isIntOperator(op) => FunType(Nil, List(("", IntType), ("", IntType)), IntType)
    case _ if isBOperator(op) => FunType(Nil, List(("", IntType), ("", IntType)), BooleanType)
    case _ =>
  ...
  def subst(tp: Type, from: Map[String, Type]): Type = tp match {
    case FunType(reftp, args, rtp) => FunType(reftp map(subst(_, from)), args map(arg => (arg._1, subst(arg._2, from))), subst(rtp, from))
    case BlockType(tps, tags) => BlockType(tps map(subst(_, from)), tags)
    case RefType(tp) if from.isDefinedAt(tp) => from(tp)
    case _ => tp
  }
  ...
  def typeConforms(tp: Type, pt: Type)(env: Env, pos: Pos): Type = (tp, pt) match {
    ...
    case (FunType(reftp1, args1, rtp1), FunType(reftp2, args2, rtp2)) if args1.length != args2.length =>
      error(s"error invalid number of arguments got ${args2.length} expected ${args1.length}", pos)
      FunType(reftp1, args1 map { x => (x._1, ErrorType) }, errorType(rtp2)) // ErrorType to avoid more errors
    case (FunType(reftp1, args1, rtp1), FunType(reftp2, args2, rtp2)) if reftp1.length != reftp2.length =>
      error(s"error invalid number of generic types got ${reftp2.length} expected ${reftp1.length}", pos)
      FunType(reftp1, args2 map { x => (x._1, ErrorType) }, errorType(rtp2))
    case (FunType(reftp1, args1, rtp1), FunType(reftp2, args2, rtp2)) =>
      val from = (reftp1 zip reftp2).map {
        case (RefType(tp), pt) => (tp, pt)
      }.toMap
      val sargs1 = args1 map { case (n1, a1) => (n1, subst(a1, from)) }
      val sargs2 = args2

      val srtp1 = subst(rtp1, from)
      val srtp2 = rtp2

      val (nargs, aErr) = ignoreErrors { // def foo(x: Int) = x; val g: Boolean => Int = f; 4
        typeConform(sargs2, sargs1)(env, pos)
      }

      val (nrtp, nErr) = ignoreErrors {
        typeConforms(srtp1, srtp2)(env, pos)
      }

      if (nErr > 0 && srtp1 == UnknownType) {
        error("return type required", pos)
        FunType(reftp1, nargs, ErrorType)
      } else if (aErr + nErr > 0) {
        val (pt, nErr) = ignoreErrors {
          typeWellFormed(FunType(reftp2, nargs, nrtp))(env, pos)
        }
        if (nErr > 0) { // def fun[U](x: V) = x; fun[Int](x)
          typeWellFormed(FunType(reftp2, nargs, nrtp))(env, pos)
        } else {
          error(s"type mismatch;\nfound   : ${FunType(reftp1, sargs1, srtp1)}\nexpected: ${FunType(reftp2, nargs, srtp2)}", pos)
        }
        FunType(reftp1, nargs map { x => (x._1, ErrorType) }, ErrorType)
      } else {
        FunType(reftp1, nargs, nrtp)
      }
    case (BlockType(List(pt), tags), FunType(Nil, List((_, tp)), rte)) if tags(BlockTag.Array.id) =>
   ...
   * UnknownType.
   * The presence of an ErrorType means that an error has been raised about it already
   */
  def typeWellFormed(tp: Type)(env: Env, pos: Pos): Type = tp match {
    case UnknownType => error("malformed type", pos); ErrorType
    case FunType(reftp, args, rte) =>
      FunType(reftp, args map { case (n, tp) => (n, typeWellFormed(tp)(env, pos)) }, typeWellFormed(rte)(env, pos))
    case BlockType(tps, tags) => BlockType(tps map(typeWellFormed(_)(env, pos)), tags)
    case RefType(name) => env(name) match {
      case Some(_) => tp
      case None =>
  ...
  def typeInfer(exp: N.Tree, pt: Type)(env: Env): S.Tree = exp match {
    ...
    case N.Ref(x) =>
      env(x) match {
        ...
        case _ =>
          ...
          S.Ref(new Symbol(x)).withType(errorType(pt))
      }
    ...
    case N.LetRec(funs, body) =>
      ...
      val (nfuns, nftps) = ((List.empty[S.FunDef], List.empty[(Symbol,Type)]) /: (funs zip ftps)) {
        case ((nfuns, nftps), (fd, (_, ftp))) =>
          val nfd = typeCheck(fd, ftp)(nenv.withValsSym(nftps))
          (nfd::nfuns, (nfd.name, nfd.tp)::nftps)
      }

      // Type check the body
      val nbody = typeCheck(body, pt)(env.withValsSym(nftps))

      S.LetRec(nfuns.reverse, nbody).withType(nbody.tp)
    case N.App(fun, ptps, args) =>
      // Check fun type
      val nFun = typeCheck(fun, FunType(ptps, listArgType(args.length, NothingType), UnknownType))(env)

      // Handling some errors
      val ftp = nFun.tp match {
        case ftp@FunType(_, fargs, _) => // if fargs.length == args.length =>
          ftp
        case BlockType(List(tp), tags) if tags(BlockTag.Array.id) =>
          FunType(Nil, List(("", IntType)), tp)
        case tp =>
          if (tp != ErrorType) error(s"$tp does not take paramters", exp.pos)
          FunType(Nil, List.fill(args.length)(("", ErrorType)), errorType(pt))
      }
    ...
    case N.PrimAlloc(tp, args) =>
      (tp, args) match {
        ...
        case _ =>
          ...
          S.Prim(ErrorPrim, Nil).withType(ErrorType)
      }
    case N.Halt(prg) =>
  ...
  def typeCheck(fun: N.FunDef, pt: Type)(env: Env): S.FunDef = {
    ...
    val nenv = env.withVals(fun.ptps map { x => (x, ExistsType)})
    val atps = fun.args map { arg => (arg.name, typeWellFormed(arg.tp)(nenv, arg.pos)) }
    val nnenv = nenv.withVals(atps)

    // Type check function body
    val nfbody = typeCheck(fun.body, fun.rtp)(nnenv)

    // Type check function def
    val ftp = funType(fun.ptps, fun.args, nfbody.tp)
    val rftp = typeConforms(ftp, pt)(nnenv, fun.pos)

    val nargs = fun.args map { case N.Arg(name, tp, pos) => S.Arg(nnenv.sym(name), tp, pos) }
